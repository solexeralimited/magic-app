...
export default function DashboardPage() {
  ...
  const { driverName, clearDriver, isLoading: driverLoading } = useDriver();
  const { jobs, activeJobs, completedJobs, isLoading, mutate, updateJobStatus } = useJobs(driverName);
  const { permission, subscribed, subscribe } = usePushNotifications(driverName);
  ...
  useEffect(() => {
    if (!driverLoading && !driverName) {
      router.replace('/drivers');
    }
  }, [driverName, driverLoading, router]);
  ...
  if (driverLoading) {
    return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin" /></div>;
  }
      ...
      <Header
        ...
        rightContent={
          <div className="flex items-center gap-1">
            ...
            {/* Logout */}
            <button onClick={handleLogout} className="bg-white/20 rounded-full p-2">
        ...
        <div className="bg-blue-50 border-b border-blue-100 max-w-2xl mx-auto">
          {messages.slice(-5).map(msg => (
            <div key={msg.id} className="px-4 py-3 border-b border-blue-100 last:border-0">
              ...
              <p className="text-xs text-blue-400 mt-0.5">{new Date(msg.sentAt).toLocaleString('en-AU')}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {permission === 'default' && (
        <div className="bg-blue-600 text-white px-4 py-3">
          <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
            ...
            <button
              onClick={subscribe}
              className="bg-white text-blue-700 text-xs font-bold px-3 py-1.5 rounded-full flex-shrink-0"
            >
              Enable
            </button>
          </div>
